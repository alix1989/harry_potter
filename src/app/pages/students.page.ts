...
<div class="card-container col-md-4" *ngFor="let char of getFilteredStudents(); let i = index">
  <div class="card">
    <div class="front">
...
export class StudentsPage implements OnInit {
  students!: Characters[];
  filteredStudents!: Characters[];
  search = '';

  constructor(private charService: CharactersService) {}

  ngOnInit(): void {
    this.charService.getStudents().subscribe((c) => {
      this.students= c as any;
    });
  }

  getFilteredStudents(): Characters[] {
    if (!this.search) {
      return this.students;
    }
    const filteredArray = this.students.filter((c) =>
  ...
  onSearch() {
    this.filteredStudents = this.students.filter((c) => {
      c.name.includes(this.search);
    });
  }
}
