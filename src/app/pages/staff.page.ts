...
<div class="card-container col-md-4" *ngFor="let char of getFilteredStaff(); let i = index">
  <div class="card">
    <div class="front">
...
export class StaffPage implements OnInit {
  ...
  ngOnInit(): void {
    this.charService.getStaff().subscribe((c) => {
      this.staff= c as any;
    });
  }

  getFilteredStaff(): Characters[] {
    if (!this.search) {
      return this.staff;
    }
    const filteredArray = this.staff.filter((c) =>
  ...
  onSearch() {
    this.filteredStudents = this.staff.filter((c) => {
      c.name.includes(this.search);
    });
  }
}
